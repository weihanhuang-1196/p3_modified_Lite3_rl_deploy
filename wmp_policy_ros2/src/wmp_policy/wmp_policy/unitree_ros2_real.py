...
class UnitreeRos2Real(Node):
    ...
    def __init__(self,
            ...
            cmd_px_range= [0.4, 0.6], # check joy_stick_callback (p for positive, n for negative)
            cmd_nx_range= [0.0, 0.0], # check joy_stick_callback (p for positive, n for negative)
            cmd_py_range= [0.2, 0.6], # check joy_stick_callback (p for positive, n for negative)
            cmd_ny_range= [0.2, 0.6], # check joy_stick_callback (p for positive, n for negative)
            cmd_pyaw_range= [0.2, 0.6], # check joy_stick_callback (p for positive, n for negative)
            cmd_nyaw_range= [0.2, 0.6], # check joy_stick_callback (p for positive, n for negative)
        ...
        self.n_hist_len = 5
        self.gravity_vec = torch.tensor([[0.0, 0.0, -1.0]],device=self.model_device)
        self.wm_latent = None
        self.wm_action = None
        self.wm_is_first = torch.ones(1, device=self.model_device)
        ...
        self.trajectory_history = torch.zeros(size=(1, self.n_hist_len, 42), device = self.model_device)
        self.wm_action_history = torch.zeros(size=(1, 5, 12),
                                    device=self.model_device)

        self.wm_obs = {
            "prop": torch.zeros(1,33,device=self.model_device, dtype=torch.float),
        ...
        self.wm_feature = torch.zeros((1, 512), device=self.model_device)
    ...
    def _joy_stick_callback(self, msg):
        ...
        if self.move_by_wireless_remote:
            ...
            if rx > self.lin_vel_deadband:
                ...
                vy = 0
            self.xyyaw_command = torch.tensor([[vx, yaw, vy]], device= self.model_device, dtype= torch.float32)
    ...
    def _depth_data_callback(self, msg):
        self.depth_data = torch.tensor(msg.data, dtype=torch.float32).reshape(1, 64, 64).to(self.model_device)
    ...
    def _sport_state_change(self, mode):
        ...
        if mode == 0:
            msg.parameter = '{"name":"mcf","switch":0}'
        elif mode == 1:
            msg.parameter = '{"name":"mcf","switch":1}'
    ...
    def _get_imu_obs(self):
        ...
        imu_obs = torch.tensor([[roll, pitch, yaw]], device= self.model_device, dtype= torch.float32)
        return imu_obs
    ...
    def _get_depth_image(self):
        return self.depth_data.unsqueeze(-1).to(self.model_device)
    ...
    def get_proprio(self):
        ...
        gravity_vec = self._get_grav_vec(self.gravity_vec)
        ...
        proprio = torch.cat([ang_vel, gravity_vec, commands, 
                        dof_pos, dof_vel,
                        last_actions, 
                        ], dim=-1)
        obs_without_command = torch.concat((proprio[:,:6],
                                        proprio[:, 9:]), dim=1)
