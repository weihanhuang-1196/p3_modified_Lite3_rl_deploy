#ifndef JOYSTICK_FEIZHI_H
#define JOYSTICK_FEIZHI_H
...
class joystick_feizhi: public joystick_base
{
private:
    /* data */
public:
    joystick_feizhi(RL& user_ref, std::string name) : joystick_base(user_ref, name){};
    virtual ~joystick_feizhi(){};
    ...
    ) override
    {
        ...
        if(this->joy_msg.buttons[10] == 1)
        {
            this->user.navigation_mode_changed = true;  
        }

        if (this->joy_msg.buttons[10] == 0 && this->user.navigation_mode_changed)
        {
            this->user.navigation_mode_changed = false;
            this->user.control.navigation_mode = !this->user.control.navigation_mode;
        }

        // Only update control if the mode is navigation  or the message is from "joy" topic
        if(!this->user.control.navigation_mode && this->joy_msg.header.frame_id != "joy")
            return;
        ...
        // if (this->joy_msg.buttons[4]) this->user.control.SetGamepad(Input::Gamepad::Y);
        ...
        if(this->joy_msg.axes[7] == 1)
        {
            this->user.stand_enable_changed = true;
        }else if(this->joy_msg.axes[7] == -1)
        {
            this->user.down_enable_changed = true;
        }
        if(this->joy_msg.axes[7] == 0 && this->user.stand_enable_changed)
        {
            this->user.control.stand = 1.0f;
            this->user.stand_enable_changed = false;
            this->user.down_enable_changed = false;
        }else if(this->joy_msg.axes[7] == 0 && this->user.down_enable_changed)
        {
            this->user.control.stand = 0.0f;
            this->user.stand_enable_changed = false;
            this->user.down_enable_changed = false;
        }

        if(this->joy_msg.buttons[7] == 0)
        {
            this->user.motor_enable_changed = true;  
        }

        if (this->joy_msg.buttons[7] == 1 && this->user.motor_enable_changed)
        {
...
class feizhiFactory : public JoystickFactory
{
public:
    feizhiFactory(const std::string& name) : name_(name) {}
    std::shared_ptr<joystick_base> CreateJoystick(RL &context, const std::string &state_name) override
    {
        return std::make_shared<joystick_feizhi>(context, state_name);
    }
    std::string GetType() const override { return "feizhi"; }
...
REGISTER_JOYSTICK_FACTORY(feizhiFactory, "feizhi")



#endif  // JOYSTICK_FEIZHI_H