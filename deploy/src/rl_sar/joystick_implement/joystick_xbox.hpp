#ifndef JOYSTICK_XBOX_H
#define JOYSTICK_XBOX_H
...
class joystick_xbox: public joystick_base
{
private:
    /* data */
public:
    joystick_xbox(RL& user_ref, std::string name) : joystick_base(user_ref, name){};
    virtual ~joystick_xbox(){};
...
class xboxFactory : public JoystickFactory
{
public:
    xboxFactory(const std::string& name) : name_(name) {}
    std::shared_ptr<joystick_base> CreateJoystick(RL &context, const std::string &state_name) override
    {
        return std::make_shared<joystick_xbox>(context, state_name);
    }
    std::string GetType() const override { return "xbox"; }
...
REGISTER_JOYSTICK_FACTORY(xboxFactory, "xbox")



#endif  // JOYSTICK_XBOX_H