...
RL_Sim::RL_Sim(int argc, char **argv)
{
    ...
    // test
    this->test_publisher = ros2_node->create_publisher<robot_msgs::msg::RobotCommand>(
        this->ros_namespace + "test", rclcpp::SystemDefaultsQoS());
...
)
{
    ...
    // if(this->joy_msg.axes[1] >= 0.5f)
    //     this->control.x = 0.5f;
    // else if (this->joy_msg.axes[1] <= - 0.5f)
    //     this->control.x = -0.5f;
    // else
...
void RL_Sim::RunModel()
{
    ...
    // robot_msgs::msg::RobotCommand str_msg;
    // this->test_publisher->publish(str_msg);
