...
RL_Sim_WMP::RL_Sim_WMP(int argc, char **argv)
{
    ...
    // loop
    this->loop_control = std::make_shared<LoopFunc>("loop_control", this->params.Get<float>("control_dt"), std::bind(&RL_Sim_WMP::RobotControl, this), std::vector<int>{5});
    // this->loop_rl = std::make_shared<LoopFunc>("loop_rl", this->params.Get<float>("dt") * this->params.Get<int>("decimation"), std::bind(&RL_Sim_WMP::RunModel, this),std::vector<int>{4});
    ...
    this->loop_plot = std::make_shared<LoopFunc>("loop_plot", 0.001, std::bind(&RL_Sim_WMP::Plot, this));
    ...
    std::cout << LOGGER::INFO << "RL_Sim_WMP start" << std::endl;
...
#endif
    std::cout << LOGGER::INFO << "RL_Sim_WMP exit" << std::endl;
}
...
void RL_Sim_WMP::InitJointStateMsg()
{
...
void RL_Sim_WMP::CmdvelCallback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
    this->cmd_vel = *msg;
}
...
void RL_Sim_WMP::InitTopics() {
   ...
   this->depth_image_normalized_publisher = this->ros2_node->create_publisher<sensor_msgs::msg::Image>(
    ...
    robot_output_dof_pos_info->extra_callback = [this, robot_output_dof_pos_info] () {
        ...
        // output_dof_pos_queue.push(std::vector<float>(this->output_dof_pos.size(), 0.0f));
...
sensor_msgs::msg::Image RL_Sim_WMP::depthVectorToGrayImage(
...
std::vector<float> RL_Sim_WMP::depth_image_to_vector(const std::vector<uint8_t>& data,int src_width, int src_height, int dst_width, int dst_height)
{
   ...
   // 计算裁剪后像素
    ...
    for (int i = 0; i < depth_meters.rows; i++) {
        ...
        for (int j = 0; j < depth_meters.cols; j++) {
            float& d = row_ptr[j];
            if (std::isnan(d) || d <= 0.0f) d = max_depth;       // 无效值设为最大可视深度
            else if (std::isinf(d)) d = max_depth;               // Inf 也设为最大深度
            else d = std::clamp(d, min_depth, max_depth);        // 其他值裁剪到 [min_depth,max_depth]
        }
    }
...
void RL_Sim_WMP::printDepthAsciiNormalized(
    const float* depth,
    int width,
    int height,
    int stride_x,
