...
setup_onnx_runtime() {
    print_header "[Setting up ONNX Runtime]"

    SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
    
    DOWNLOAD_SCRIPT="${SCRIPT_DIR}/scripts/download_inference_runtime.sh"
    if [ -f "$DOWNLOAD_SCRIPT" ]; then
        print_info "Using generic download script for ONNX only..."
        ...
        print_warning "No download script found, attempting direct ONNX installation..."
        install_onnx_directly
    fi
}
