...
std::vector<float> RL::ComputeObservation()
{
    ...
    for (const std::string &observation : this->params.Get<std::vector<std::string>>("observations"))
    {
        ...
        else if(observation == "sin")
        {
            std::vector<float> dof_pos_rel = this->obs.dof_pos - this->params.Get<std::vector<float>>("default_dof_pos");
            std::vector<float> sin_dof_pos_rel(dof_pos_rel.size());
            std::transform(dof_pos_rel.begin(), dof_pos_rel.end(),
                            sin_dof_pos_rel.begin(),
                            [](float x) { return std::sin(x); });
            obs_list.push_back(sin_dof_pos_rel);
            
        }
        else if(observation == "cos")
        {
            std::vector<float> dof_pos_rel = this->obs.dof_pos - this->params.Get<std::vector<float>>("default_dof_pos");
            std::vector<float> cos_dof_pos_rel(dof_pos_rel.size());
            std::transform(dof_pos_rel.begin(), dof_pos_rel.end(),
                            cos_dof_pos_rel.begin(),
                            [](float x) { return std::cos(x); });
            obs_list.push_back(cos_dof_pos_rel);
        }
        // ============= Other Observations =============
...
void RL::InitObservations()
{
    ...
    this->znear = this->params.Get<float>("znear", 0.0);
    this->zfar = this->params.Get<float>("zfar", 2.0f);
    this->camera_znear = this->params.Get<float>("camera_znear", 0.05f);
    this->camera_zfar = this->params.Get<float>("camera_zfar", 40.0f);
    this->input_image = std::vector<float>(64*64, 0.0f);
...
void RL::InitControl()
{
    ...
    // this->motor_enable_changed = false;
