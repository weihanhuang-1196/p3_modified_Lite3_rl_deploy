#ifndef JOYSTICK_SONY_H
#define JOYSTICK_SONY_H
...
class joystick_sony: public joystick_base
{
private:
    /* data */
public:
    joystick_sony(RL& user_ref, std::string name) : joystick_base(user_ref, name){};
    virtual ~joystick_sony(){};
...
class SonyFactory : public JoystickFactory
{
public:
    SonyFactory(const std::string& name) : name_(name) {}
    std::shared_ptr<joystick_base> CreateJoystick(RL &context, const std::string &state_name) override
    {
        return std::make_shared<joystick_sony>(context, state_name);
    }
    std::string GetType() const override { return "sony"; }
...
REGISTER_JOYSTICK_FACTORY(SonyFactory, "sony")

#endif  // JOYSTICK_SONY_H