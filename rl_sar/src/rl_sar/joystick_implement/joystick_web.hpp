#ifndef JOYSTICK_WEB_H
#define JOYSTICK_WEB_H
...
class joystick_web: public joystick_base
{
private:
    /* data */
public:
    joystick_web(RL& user_ref, std::string name) : joystick_base(user_ref, name){};
    virtual ~joystick_web(){};
    ...
    ) override
    {
        ...
        if (this->joy_msg.buttons[6]) this->user.control.SetGamepad(Input::Gamepad::RB);
        ...
        if(this->joy_msg.buttons[5] == 0 && this->user.prev_policy_changed == true)
        {
...
class webFactory : public JoystickFactory
{
public:
    webFactory(const std::string& name) : name_(name) {}
    std::shared_ptr<joystick_base> CreateJoystick(RL &context, const std::string &state_name) override
    {
        return std::make_shared<joystick_web>(context, state_name);
    }
...
REGISTER_JOYSTICK_FACTORY(webFactory, "web")



#endif  // JOYSTICK_WEB_H