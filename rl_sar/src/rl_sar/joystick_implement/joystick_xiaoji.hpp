#ifndef JOYSTICK_XIAOJI_H
#define JOYSTICK_XIAOJI_H
...
class joystick_xiaoji: public joystick_base
{
private:
    /* data */
public:
    joystick_xiaoji(RL& user_ref, std::string name) : joystick_base(user_ref, name){};
    virtual ~joystick_xiaoji(){};
    ...
    ) override
    {
        ...
        if (this->joy_msg.buttons[9] == 0 && this->user.motor_enable_changed)
        {
...
class xiaojiFactory : public JoystickFactory
{
public:
    xiaojiFactory(const std::string& name) : name_(name) {}
    std::shared_ptr<joystick_base> CreateJoystick(RL &context, const std::string &state_name) override
    {
        return std::make_shared<joystick_xiaoji>(context, state_name);
    }
    std::string GetType() const override { return "xiaoji"; }
...
REGISTER_JOYSTICK_FACTORY(xiaojiFactory, "xiaoji")



#endif  // JOYSTICK_XIAOJI_H