...
#ifndef PANDA3V2_FSM_HPP
#define PANDA3V2_FSM_HPP

#include "fsm.hpp"
#include "rl_sdk.hpp"

namespace panda3v2_fsm
{
...
public:
    ...
    void Enter() override
    {
        ...
        try
        {
            ...
            rl.policy_manager.SwitchPolicy(this->state_name_);
            rl.InitRL();
            rl.now_state = *fsm_state;
        }
    ...
    std::string CheckChange() override
    {
        ...
        // }
        else if (rl.next_policy_switch.exchange(false))
        {
            rl.policy_manager.SwitchNextPolicy();
            rl.InitRL();
        }
        else if (rl.prev_policy_switch.exchange(false))
        {
            rl.policy_manager.SwitchPrevPolicy();
            rl.InitRL();
        }
...
class Panda3v2FSMFactory : public FSMFactory
{
public:
    Panda3v2FSMFactory(const std::string& initial) : initial_state_(initial) {}
    std::shared_ptr<FSMState> CreateState(void *context, const std::string &state_name) override
    {
        RL *rl = static_cast<RL *>(context);
        if (state_name == "RLFSMStatePassive")
            return std::make_shared<panda3v2_fsm::RLFSMStatePassive>(rl);
        else if (state_name == "RLFSMStateGetUp")
            return std::make_shared<panda3v2_fsm::RLFSMStateGetUp>(rl);
        else if (state_name == "RLFSMStateGetDown")
            return std::make_shared<panda3v2_fsm::RLFSMStateGetDown>(rl);
        else if (state_name == "RLFSMStateRLWalk")
            return std::make_shared<panda3v2_fsm::RLFSMStateRLWalk>(rl);
    }
    std::string GetType() const override { return "panda3v2"; }
...
REGISTER_FSM_FACTORY(Panda3v2FSMFactory, "RLFSMStatePassive")

#endif // PANDA3V2_FSM_HPP
