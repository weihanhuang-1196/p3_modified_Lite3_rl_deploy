...
#if defined(MOTOR_POLICY)
    // motor policy
    #include <deque>
    std::unique_ptr<InferenceRuntime::Model> loop_motor_policy;
    std::deque<std::vector<float>> errs_history;
    std::deque<std::vector<float>> qd_history;

    void InitMotorPolicy();
